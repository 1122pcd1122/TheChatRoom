...
public class FriendDaoImpl implements FriendDao {
    ...
    @Override
    public boolean insertFriend(String userId, String fiendId) {

        try {
            Connection connection = JdbcUtil.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement("select * from user where id = ?");
            preparedStatement.setString(1,fiendId);
            ResultSet resultSet = preparedStatement.executeQuery();
            if (resultSet.next()){
                PreparedStatement preparedStatement1 = connection.prepareStatement("insert into friends(id,friendId) value(?,?)");
                preparedStatement1.setString(1,userId);
                preparedStatement1.setString(2,fiendId);
